...
<Aside type="tip" title="Important">
  ...
  attempt to render the template anyway. We recommend to always include the
...
<Aside type="note">
  The `filename` key is **required** when used alongside the `matrix` key, to
  allow whiskers to generate multiple files.
</Aside>
