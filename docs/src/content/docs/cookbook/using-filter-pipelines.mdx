...
description: How to get a value, such as a color's hex representation, at the end of a filter pipeline.
---

Let's say we want to modify a color and then insert its hex into the template:
...
Getting the hex string from a color at the end of a filter pipeline is such a common need that we added a dedicated `hex` filter for it:
